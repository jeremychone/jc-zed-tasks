...
const TMUX_LIST_FORMAT: &str = "#{?session_attached,ATTACHED,DETACHED} #S:#I.#P #{window_name} [#{pane_title}] #{pane_current_path} #{pane_current_command} #{session_id} #{window_id}";
...
pub fn list_sessions() -> Result<TmuxSessions> {
	...
	for line in output.lines() {
		...
		let session = if let Some(s) = sessions.iter_mut().find(|s| s.name == parts.s_name) {
			s
		} else {
		...
		let window = if let Some(w) = session.windows.iter_mut().find(|w| w.index == parts.w_idx) {
			w
		} else {
...
fn parse_line(line: &str) -> Option<LineParts> {
	// Format: ATTACHED/DETACHED session_name:win_idx.pane_idx win_name [pane_title] path command session_id window_id
	...
	// rest: win_name [pane_title] path command session_id window_id
	...
	// tail: path command session_id window_id
	let (tail_path_cmd, w_id) = tail.rsplit_once(' ')?;
	let (path_cmd, s_id) = tail_path_cmd.rsplit_once(' ')?;
